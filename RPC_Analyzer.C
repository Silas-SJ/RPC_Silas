...
Cut's reference used was from this article:
https://iopscience.iop.org/article/10.1088/1748-0221/15/10/P10017/pdf
*/
...
void RPC_Analyzer() {
 ...
 /// Create a output file to seve histograms 
  ...
  hist["n_RecHit_Barrel"] = new TH1F("RecHit_barrel", "RecHits barrel", 35, 0.0, 35.0);
  hist["n_RecHit_Barrel"]->GetXaxis()->SetTitle("Number of hits barrel");
  hist["n_RecHit_Barrel"]->GetYaxis()->SetTitle("Entries"); 
  
  hist["n_RecHit_Barrel_lowPT"] = new TH1F("RecHit_Barrel_lowPT", "RecHits barrel", 35, 0.0, 35.0);
  hist["n_RecHit_Barrel_lowPT"]->GetXaxis()->SetTitle("Number of hits barrel");
  hist["n_RecHit_Barrel_lowPT"]->GetYaxis()->SetTitle("Entries"); 
  
  hist["n_RecHit_Endcap"] = new TH1F("RecHit_Endcap", "RecHits endcap", 35, 0.0, 35.0);
  hist["n_RecHit_Endcap"]->GetXaxis()->SetTitle("Number of hits endcap station");
  hist["n_RecHit_Endcap"]->GetYaxis()->SetTitle("Entries");   
  
  hist["n_RecHit_Endcap_lowPT"] = new TH1F("RecHit_Endcap_lowPT", "RecHits endcap", 35, 0.0, 35.0);
  hist["n_RecHit_Endcap_lowPT"]->GetXaxis()->SetTitle("Number of hits endcap station");
  hist["n_RecHit_Endcap_lowPT"]->GetYaxis()->SetTitle("Entries");   
  
  hist["n_RecHit_clusterSize"] = new TH1F("RecHit_cluster_Size", "Cluster Size", 70, 0.0, 70.0);
  hist["n_RecHit_clusterSize"]->GetXaxis()->SetTitle("Cluster Size");
  hist["n_RecHit_clusterSize"]->GetYaxis()->SetTitle("Entries"); 
  
  hist["n_RecHit_clusterSize_Barrel"] = new TH1F("cluster_Size_Barrel", "Cluster Size barrel", 22, 0.0, 22.0);
  hist["n_RecHit_clusterSize_Barrel"]->GetXaxis()->SetTitle("Cluster Size barrel");
  hist["n_RecHit_clusterSize_Barrel"]->GetYaxis()->SetTitle("Entries"); 
  
  hist["n_RecHit_clusterSize_Barrel_lowPT"] = new TH1F("clusterSize_Barrel_lowPT", "Cluster Size barrel", 22, 0.0, 22.0);
  hist["n_RecHit_clusterSize_Barrel_lowPT"]->GetXaxis()->SetTitle("Cluster Size barrel");
  hist["n_RecHit_clusterSize_Barrel_lowPT"]->GetYaxis()->SetTitle("Entries");
  
  hist["n_RecHit_clusterSize_Endcap"] = new TH1F("cluster_Size_Endcap", "Cluster Size endcap station", 22, 0.0, 22.0);
  hist["n_RecHit_clusterSize_Endcap"]->GetXaxis()->SetTitle("Cluster Size endcap station");
  hist["n_RecHit_clusterSize_Endcap"]->GetYaxis()->SetTitle("Entries"); 
  
  hist["n_RecHit_clusterSize_Endcap_lowPT"] = new TH1F("cluster_Size_Endcap_lowPT", "Cluster Size endcap station", 22, 0.0, 22.0);
  hist["n_RecHit_clusterSize_Endcap_lowPT"]->GetXaxis()->SetTitle("Cluster Size endcap station");
  hist["n_RecHit_clusterSize_Endcap_lowPT"]->GetYaxis()->SetTitle("Entries"); 
  ...
  while (reader.Next()) {
     // std::cout << "Event: " << reader.GetCurrentEntry() << std::endl;
        ...
        for(std::size_t n_i =0; n_i<number_muon; ++n_i){ 
           ...
           // For low Pt
           else if( std::abs(mu_eta[n_i]) > Min_eta_barrel &&  std::abs(mu_eta[n_i]) < Max_eta_barrel && mu_pt[n_i]<Min_pt_barrel){
                   hist["n_RecHit_clusterSize_Barrel_lowPT"]->Fill(n_rpcRecHit_clusterSize[n_i]);
                   hist["n_RecHit_Barrel_lowPT"]->Fill(n_rpcRecHit[n_i]); 
           }   
           ...
           // For low Pt
           else if( std::abs(mu_eta[n_i]) > Min_eta_endcap &&  std::abs(mu_eta[n_i])< Max_eta_endcap && mu_pt[n_i]<Min_pt_endcap){
                   hist["n_RecHit_clusterSize_Endcap_lowPT"]->Fill(n_rpcRecHit_clusterSize[n_i]);
                   hist["n_RecHit_Endcap_lowPT"]->Fill(n_rpcRecHit[n_i]); 
           }
 ...
 /// Oly use it for save useful histograms in png format, for example:     
  auto c1 = new TCanvas("c1", "");
       ...
       c1->SaveAs("RecHit_clusterSize.png");
       
       hist["n_RecHit_Barrel"]->Draw();
       c1->SaveAs("RecHit_barrel.png");
       hist["n_RecHit_Barrel_lowPT"]->Draw();
       c1->SaveAs("RecHit_barrel_lowPT.png");
       
       hist["n_RecHit_Endcap"]->Draw();
       c1->SaveAs("RecHit_endcap.png");    
       hist["n_RecHit_Endcap_lowPT"]->Draw();
       c1->SaveAs("RecHit_endcap_lowPT.png");
              
       hist["n_RecHit_clusterSize_Barrel"]->Draw();
       c1->SaveAs("clusterSize_barrel.png");
       hist["n_RecHit_clusterSize_Barrel_lowPT"]->Draw();
       c1->SaveAs("clusterSize_barrel_lowPT.png");
       
       hist["n_RecHit_clusterSize_Endcap"]->Draw();
       c1->SaveAs("clusterSize_endcap.png");
       hist["n_RecHit_clusterSize_Endcap_lowPT"]->Draw();
       c1->SaveAs("clusterSize_endcap_lowPT.png");
